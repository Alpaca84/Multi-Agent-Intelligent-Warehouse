...
async def test_redis_cache_service():
    ...
    try:
        from inventory_retriever.caching.redis_cache_service import (
...
async def test_cache_manager():
    ...
    try:
        from inventory_retriever.caching.cache_manager import (
            CacheManager, CachePolicy, CacheWarmingRule, EvictionStrategy
        )
        from inventory_retriever.caching.redis_cache_service import CacheType
        from inventory_retriever.caching.redis_cache_service import get_cache_service
...
async def test_cache_integration():
    ...
    try:
        from inventory_retriever.caching.cache_integration import (
...
async def test_cache_monitoring():
    ...
    try:
        from inventory_retriever.caching.cache_monitoring import (
            CacheMonitoringService, AlertLevel
        )
        from inventory_retriever.caching.redis_cache_service import get_cache_service
        from inventory_retriever.caching.cache_manager import get_cache_manager
...
async def main():
    ...
    if passed == len(test_results):
        print("🎉 All cache system tests passed! The caching system is working correctly.")
    else:

...
class AgentConfigLoader:
    ...
    def __init__(self, config_dir: Optional[Path] = None):
        ...
        if config_dir is None:
            ...
            if project_root is None:
                # Fallback: assume we are in src/api/services and root is 3 levels up
                # src/api/services -> src/api -> src -> root
                potential_root = current.parents[2]
                if (potential_root / "data").exists():
                    project_root = potential_root
                    logger.info(
                        f"Project root markers not found, using fallback: {project_root}"
                    )
    ...
    def load_agent_config(self, agent_name: str) -> AgentConfig:
        ...
        try:
            ...
            missing_persona_fields = [
                field for field in required_persona_fields if field not in persona_data
            ]
    ...
    def get_all_agents(self) -> list[str]:
        ...
        return [
            f.stem.replace("_agent", "") for f in self.config_dir.glob("*_agent.yaml")
        ]

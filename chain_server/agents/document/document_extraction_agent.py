...
from chain_server.agents.document.models.document_models import (
    DocumentResponse, DocumentUpload, ProcessingStage, ProcessingStatus,
...
class DocumentExtractionAgent:
    ...
    async def process_document(
        ...
        try:
            ...
            # Layout detection
            layout_result = await self.layout_detector.detect_layout(preprocessing_result)
            ...
            embedding_result = await self.embedding_service.generate_and_store_embeddings(
                document_id,
                llm_result["structured_data"],
                entities,
                document_type
            ...
            logger.info(f"Document processing completed for {document_id} in {processing_time_ms}ms")
            return result
    ...
    async def get_processing_status(self, document_id: str) -> Dict[str, Any]:
        ...
        current_stage = processing_state["stages_completed"][-1] if processing_state["stages_completed"] else ProcessingStage.PREPROCESSING
        ...
        return {
            ...
            "total_stages": 6,
            "progress_percentage": (len(processing_state["stages_completed"]) / 6) * 100,
            "estimated_completion": processing_state["start_time"].timestamp() + 60,  # 60 seconds estimate
            "errors": processing_state["errors"]
        }

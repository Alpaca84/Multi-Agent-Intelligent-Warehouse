...
class RoutingDecision(BaseModel):
    ...
    wms_integration_data: Optional[Dict[str, Any]] = Field(None, description="WMS integration data")
    human_review_required: bool = Field(False, description="Whether human review is required")
...
class DocumentSearchMetadata(BaseModel):
    ...
    key_entities: Dict[str, Any] = Field(default_factory=dict, description="Key entities extracted")
...
class DocumentResultsResponse(BaseModel):
    ...
    extraction_results: List[ExtractionResult] = Field(..., description="Extraction results from all stages")
    ...
    search_metadata: Optional[DocumentSearchMetadata] = Field(None, description="Search metadata")
...
class DocumentSearchRequest(BaseModel):
    ...
    quality_threshold: Optional[float] = Field(None, ge=0, le=5, description="Minimum quality score")
...
class DocumentSearchResult(BaseModel):
    ...
    key_entities: Dict[str, Any] = Field(default_factory=dict, description="Key entities")
...
class DocumentResponse(BaseModel):
    ...
    processing_status: Optional[DocumentStatus] = Field(None, description="Processing status if applicable")

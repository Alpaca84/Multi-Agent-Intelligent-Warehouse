...
class QualityScorer:
    ...
    async def score_document(
        ...
        try:
            ...
            completeness_score = await self._calculate_completeness_score(judge_result, entities, document_type)
            accuracy_score = await self._calculate_accuracy_score(judge_result, entities, document_type)
            consistency_score = await self._calculate_consistency_score(judge_result, entities, document_type)
            readability_score = await self._calculate_readability_score(judge_result, entities, document_type)
            ...
            feedback = await self._generate_feedback(judge_result, entities, document_type)
            recommendations = await self._generate_recommendations(judge_result, entities, document_type)
            ...
            logger.info(f"Quality scoring completed with overall score: {overall_score:.2f}")
    ...
    async def _calculate_completeness_score(
        ...
        try:
            ...
            entity_completeness = await self._calculate_entity_completeness(entities, document_type)
            
            # Calculate field completeness
            field_completeness = await self._calculate_field_completeness(entities, document_type)
    ...
    async def _calculate_entity_completeness(self, entities: Dict[str, Any], document_type: str) -> float:
    ...
    async def _calculate_field_completeness(self, entities: Dict[str, Any], document_type: str) -> float:
                ...
                if isinstance(entity_list, list):
                    for entity in entity_list:
                        if isinstance(entity, dict):
                            ...
                            if entity_type == "financial" and self._is_valid_financial(value):
                                correct_types += 1
                            elif entity_type == "temporal" and self._is_valid_date(value):
                                correct_types += 1
                            elif entity_type == "identifier" and self._is_valid_identifier(value):
                                correct_types += 1
                            elif entity_type in ["contact", "product", "address"]:
                                correct_types += 1  # Assume correct for non-numeric types
    ...
    async def _generate_feedback(
        ...
        try:
            ...
            if completeness.get("issues"):
                feedback_parts.append(f"Completeness Issues: {', '.join(completeness['issues'])}")
            
            # Add accuracy feedback
            accuracy = judge_result.get("accuracy", {})
            if accuracy.get("calculation_errors"):
                feedback_parts.append(f"Calculation Errors: {', '.join(accuracy['calculation_errors'])}")
            
            # Add compliance feedback
            compliance = judge_result.get("compliance", {})
            if compliance.get("compliance_issues"):
                feedback_parts.append(f"Compliance Issues: {', '.join(compliance['compliance_issues'])}")
            
            # Add quality feedback
            quality = judge_result.get("quality", {})
            if quality.get("anomalies"):
                feedback_parts.append(f"Quality Anomalies: {', '.join(quality['anomalies'])}")
            
            return ". ".join(feedback_parts) if feedback_parts else "No specific issues identified."
    ...
    async def _calculate_confidence(
        ...
        try:
            ...
            entity_confidence = total_confidence / total_entities if total_entities > 0 else 0.0

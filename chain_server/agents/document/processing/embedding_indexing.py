...
class EmbeddingIndexingService:
    ...
    async def generate_and_store_embeddings(
        ...
        try:
            ...
            # Prepare metadata
            metadata = await self._prepare_metadata(document_id, structured_data, entities, document_type)
            
            # Store in vector database
            storage_result = await self._store_in_milvus(document_id, embeddings, metadata)
                ...
                if isinstance(entity_list, list):
                    for entity in entity_list:
                        if isinstance(entity, dict) and entity.get('value'):
                            text_content.append(f"{entity.get('name', '')}: {entity['value']}")
    ...
    async def _generate_mock_embeddings(self, text_content: List[str]) -> List[List[float]]:
    ...
    async def _initialize_milvus(self):
        ...
        try:
            ...
            logger.info(f"Initializing Milvus connection to {self.milvus_host}:{self.milvus_port}")

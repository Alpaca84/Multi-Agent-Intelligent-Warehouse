...
class EntityExtractor:
    ...
    async def extract_entities(
        ...
        try:
            ...
            for field_name, field_data in extracted_fields.items():
                entity = await self._extract_field_entity(field_name, field_data, document_type)
                if entity:
    ...
    async def _extract_field_entity(
        ...
        try:
            ...
            # Extract metadata
            metadata = await self._extract_entity_metadata(value, entity_type, document_type)
    ...
    def _determine_entity_type(self, field_name: str, value: str) -> str:
        ...
        # Financial entities
        if any(keyword in field_name_lower for keyword in ["amount", "total", "price", "cost", "value"]):
            return "financial"
        ...
        # Temporal entities
        elif any(keyword in field_name_lower for keyword in ["date", "time", "due", "created", "issued"]):
            return "temporal"
        ...
        # Address entities
        elif any(keyword in field_name_lower for keyword in ["address", "location", "street", "city", "state", "zip"]):
            return "address"
        elif re.search(r'\d+\s+\w+\s+(street|st|avenue|ave|road|rd|boulevard|blvd)', value_lower):
            return "address"
        
        # Identifier entities
        elif any(keyword in field_name_lower for keyword in ["number", "id", "code", "reference"]):
            return "identifier"
        elif re.match(r'^[A-Z]{2,4}-\d{3,6}$', value.strip()):
            return "identifier"
        
        # Contact entities
        elif any(keyword in field_name_lower for keyword in ["name", "company", "vendor", "supplier", "customer"]):
            return "contact"
        elif "@" in value and "." in value:
            return "contact"
        
        # Product entities
        elif any(keyword in field_name_lower for keyword in ["item", "product", "description", "sku"]):
            return "product"
        ...
        if len(parts) >= 2:
            ...
            if len(parts) >= 3:
                ...
                if zip_match:
                    ...
                    components["state"] = state_zip.replace(zip_match.group(1), '').strip()
    ...
    async def _extract_product_entities(self, item: Dict[str, Any]) -> List[ExtractedEntity]:
    ...
    def _categorize_entity(self, entity: ExtractedEntity, entities: Dict[str, Any]) -> Dict[str, Any]:

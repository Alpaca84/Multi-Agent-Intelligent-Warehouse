...
from chain_server.services.iot.integration_service import iot_service
from adapters.iot.base import SensorType, EquipmentStatus, SensorReading, Equipment, Alert
...
class IoTConnectionConfig(BaseModel):
    iot_type: str = Field(..., description="Type of IoT system (equipment_monitor, environmental, safety_sensors, asset_tracking)")
...
async def add_iot_connection(
    ...
    try:
        ...
        if success:
            ...
            raise HTTPException(status_code=400, detail="Failed to connect to IoT system")
...
async def get_sensor_readings(connection_id: str, request: SensorReadingsRequest = Depends()):
...
async def get_equipment_status(connection_id: str, request: EquipmentStatusRequest = Depends()):
    """Get equipment status from a specific IoT connection."""
    try:
        equipment = await iot_service.get_equipment_status(connection_id, request.equipment_id)
...
async def iot_health_check():
    ...
    try:
        status = await iot_service.get_connection_status()
        return {
            "status": "healthy" if any(conn.get("connected", False) for conn in status.values()) else "unhealthy",
            "connections": status,

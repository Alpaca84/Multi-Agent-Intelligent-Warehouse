...
async def get_inventory_item(sku: str):
    ...
    try:
        await sql_retriever.initialize()
        item = await InventoryQueries().get_item_by_sku(sql_retriever, sku)
...
async def create_inventory_item(item: InventoryItem):
    ...
    try:
        await sql_retriever.initialize()
        await InventoryQueries().add_item(
            sql_retriever,
            item.sku,
            item.name,
            item.quantity,
...
@router.put("/inventory/{sku}", response_model=InventoryItem)
async def update_inventory_item(sku: str, update: InventoryUpdate):
    ...
    try:
        ...
        # Get current item
        current_item = await InventoryQueries().get_item_by_sku(sql_retriever, sku)
        if not current_item:
            raise HTTPException(status_code=404, detail=f"Inventory item with SKU {sku} not found")
        ...
        await InventoryQueries().update_item_quantity(sql_retriever, sku, quantity)
        ...
        # Return updated item
        updated_item = await InventoryQueries().get_item_by_sku(sql_retriever, sku)
        return InventoryItem(

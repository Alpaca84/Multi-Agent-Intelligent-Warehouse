...
from chain_server.services.attendance.integration_service import attendance_service
from adapters.time_attendance.base import AttendanceRecord, BiometricData, AttendanceType, AttendanceStatus, BiometricType
...
class AttendanceSystemRequest(BaseModel):
    ...
    device_type: str = Field(..., description="System type (biometric_system, card_reader, mobile_app)")
    ...
    additional_params: Optional[Dict[str, Any]] = Field(None, description="Additional system parameters")
...
async def create_system(request: AttendanceSystemRequest):
    ...
    try:
        from adapters.time_attendance.base import AttendanceConfig
        ...
        if success:
            return {"message": f"Attendance system '{request.system_id}' created successfully"}
        else:
            raise HTTPException(status_code=400, detail="Failed to create attendance system")
...
async def create_attendance_record(system_id: str, record: AttendanceRecordModel):
    ...
    try:
        ...
        success = await attendance_service.create_attendance_record(system_id, attendance_record)
        
        if success:
            return {"message": "Attendance record created successfully"}
        else:
            raise HTTPException(status_code=400, detail="Failed to create attendance record")
...
async def update_attendance_record(system_id: str, record_id: str, record: AttendanceRecordModel):
    ...
    try:
        ...
        success = await attendance_service.update_attendance_record(system_id, attendance_record)
        
        if success:
            return {"message": "Attendance record updated successfully"}
        else:
            raise HTTPException(status_code=400, detail="Failed to update attendance record")
            
    except Exception as e:
        logger.error(f"Failed to update attendance record: {e}")
        raise HTTPException(status_code=500, detail=str(e))

@router.delete("/systems/{system_id}/records/{record_id}", response_model=Dict[str, str])
async def delete_attendance_record(system_id: str, record_id: str):
    """Delete an attendance record."""
    try:
        success = await attendance_service.delete_attendance_record(system_id, record_id)
...
@router.get("/systems/{system_id}/employees/{employee_id}/summary", response_model=Dict[str, Any])
async def get_employee_attendance(
    ...
    try:
        summary = await attendance_service.get_employee_attendance(system_id, employee_id, date)
        return summary
    except Exception as e:
...
async def enroll_biometric_data(system_id: str, biometric_data: BiometricDataModel):
    ...
    try:
        ...
        if success:
            ...
            raise HTTPException(status_code=400, detail="Failed to enroll biometric data")
...
async def verify_biometric(
    ...
    try:
        employee_id = await attendance_service.verify_biometric(system_id, biometric_type, template_data)
        
        return {
            "success": employee_id is not None,
            "employee_id": employee_id,
            "message": "Biometric verification successful" if employee_id else "No match found"
        }
    except Exception as e:
...
async def health_check():
    ...
    try:
        ...
        connected_systems = sum(1 for status in systems_status.values() if status["connected"])
        syncing_systems = sum(1 for status in systems_status.values() if status["syncing"])

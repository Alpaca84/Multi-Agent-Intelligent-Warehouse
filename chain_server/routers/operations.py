...
from inventory_retriever.structured import SQLRetriever, TaskQueries
import logging
...
async def update_task(task_id: int, update: TaskUpdate):
    ...
    try:
        ...
        query = """
            ...
            RETURNING id, kind, status, assignee, payload, created_at, updated_at
        """
        result = await sql_retriever.fetch_one(query, status, assignee, payload, task_id)
...
async def get_workforce_status():
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="Failed to retrieve workforce status")

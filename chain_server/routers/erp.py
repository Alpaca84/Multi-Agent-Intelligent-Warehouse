...
from chain_server.services.erp.integration_service import erp_service
from adapters.erp.base import ERPResponse
...
async def create_connection(request: ERPConnectionRequest):
    ...
    try:
        from adapters.erp.base import ERPConnection
        ...
        if success:
            return {"message": f"ERP connection '{request.connection_id}' created successfully"}
        else:
            raise HTTPException(status_code=400, detail="Failed to create ERP connection")
...
async def get_employees(
    ...
    try:
        response = await erp_service.get_employees(connection_id, parse_filters(filters))
...
async def get_suppliers(
    ...
    try:
        response = await erp_service.get_suppliers(connection_id, parse_filters(filters))
        return ERPResponseModel(
            success=response.success,
            data=response.data,
            error=response.error,
...
@router.get("/connections/{connection_id}/purchase-orders", response_model=ERPResponseModel)
async def get_purchase_orders(
    ...
    try:
        response = await erp_service.get_purchase_orders(connection_id, parse_filters(filters))
        return ERPResponseModel(
            success=response.success,
            data=response.data,
            error=response.error,
...
@router.get("/connections/{connection_id}/sales-orders", response_model=ERPResponseModel)
async def get_sales_orders(
    ...
    try:
        response = await erp_service.get_sales_orders(connection_id, parse_filters(filters))
        return ERPResponseModel(
            success=response.success,
            data=response.data,
            error=response.error,
...
@router.get("/connections/{connection_id}/financial-data", response_model=ERPResponseModel)
async def get_financial_data(
    ...
    try:
        response = await erp_service.get_financial_data(connection_id, parse_filters(filters))
        return ERPResponseModel(
            success=response.success,
            data=response.data,
            error=response.error,
...
@router.get("/connections/{connection_id}/warehouse-data", response_model=ERPResponseModel)
async def get_warehouse_data(
    ...
    try:
        response = await erp_service.get_warehouse_data(connection_id, parse_filters(filters))
        return ERPResponseModel(
            success=response.success,
            data=response.data,
            error=response.error,
...
async def health_check():
    ...
    try:
        ...
        active_connections = sum(1 for status in connections_status.values() if status["connected"])

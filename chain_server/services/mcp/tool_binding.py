...
class ToolBindingService:
    ...
    async def bind_tools(
        ...
        try:
            ...
            relevant_tools = await self._discover_relevant_tools(query, intent, entities, context)
            ...
            logger.info(f"Bound {len(bindings)} tools to agent {agent_id} using {strategy.value} strategy")
            return bindings
    ...
    async def _performance_based_binding(
        ...
        for tool in sorted_tools[:max_tools]:
            confidence = tool.success_rate * 0.8 + (1.0 - min(tool.average_response_time / 10.0, 1.0)) * 0.2
    ...
    def _create_sequential_steps(self, bindings: List[ToolBinding], context: ExecutionContext) -> List[Dict[str, Any]]:
    ...
    def _create_parallel_steps(self, bindings: List[ToolBinding], context: ExecutionContext) -> List[Dict[str, Any]]:
    ...
    def _create_pipeline_steps(self, bindings: List[ToolBinding], context: ExecutionContext) -> List[Dict[str, Any]]:
    ...
    def _create_conditional_steps(self, bindings: List[ToolBinding], context: ExecutionContext) -> List[Dict[str, Any]]:
    ...
    def _create_fallback_steps(self, bindings: List[ToolBinding], context: ExecutionContext) -> List[Dict[str, Any]]:
    ...
    def _prepare_arguments(self, binding: ToolBinding, context: ExecutionContext) -> Dict[str, Any]:
    ...
    def _evaluate_condition(self, condition: str, results: List[ExecutionResult]) -> bool:
    ...
    async def _execute_step(self, step: Dict[str, Any], context: ExecutionContext) -> ExecutionResult:
    ...
    async def get_bindings_for_agent(self, agent_id: str) -> List[ToolBinding]:
        ...
        return [binding for binding in self.bindings.values() if binding.agent_id == agent_id]
    
    async def get_binding_statistics(self) -> Dict[str, Any]:
        """Get binding statistics."""
        total_bindings = len(self.bindings)
        active_bindings = len([b for b in self.bindings.values() if b.last_used and (datetime.utcnow() - b.last_used).days < 7])
        
        return {
            "total_bindings": total_bindings,
            "active_bindings": active_bindings,
            "execution_history_count": len(self.execution_history),
            "average_confidence": sum(b.binding_confidence for b in self.bindings.values()) / total_bindings if total_bindings > 0 else 0.0
        }

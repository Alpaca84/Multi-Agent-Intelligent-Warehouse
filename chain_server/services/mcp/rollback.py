...
from chain_server.services.mcp.base import MCPError, MCPToolBase, MCPAdapter
...
class MCPRollbackManager:
    ...
    async def _check_rollback_triggers(self):
        ...
        if self.metrics.response_time > self.config.rollback_thresholds["response_time"]:
...
class MCPToolFallback(MCPToolBase):
    ...
    def __init__(self, name: str, description: str, rollback_manager: MCPRollbackManager):
    ...
    async def execute(self, parameters: dict) -> dict:
        ...
        try:
            ...
            if self.legacy_implementation:
                self.logger.warning(f"MCP tool {self.name} failed, falling back to legacy: {e}")
    ...
    async def process(self, request: dict) -> dict:
        ...
        try:
            ...
            if self.legacy_agent:
                self.logger.warning(f"MCP agent {self.name} failed, falling back to legacy: {e}")
    ...
    async def execute_operation(self, operation: str, *args, **kwargs):
        ...
        if self.mcp_enabled:
            try:
                ...
                self.logger.warning(f"MCP operation failed, falling back to legacy: {e}")
                if self.legacy_system:
                    return await self.legacy_system.execute_operation(operation, *args, **kwargs)
                else:
                    raise MCPError(f"No fallback implementation for operation: {operation}")
        else:
            # Use legacy system
            if self.legacy_system:
                return await self.legacy_system.execute_operation(operation, *args, **kwargs)
...
async def mcp_fallback_context(rollback_manager: MCPRollbackManager):
    ...
    try:
        ...
        rollback_manager.logger.warning(f"MCP operation failed, triggering fallback: {e}")
...
async def example_usage():
    ...
    rollback_manager.register_legacy_implementation("get_inventory", legacy_get_inventory)

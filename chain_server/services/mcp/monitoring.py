        ...
        async with self._lock:
            if format == "json":
                ...
                for key, metrics in self.metrics.items():
                    ...
                    lines.append(f"{latest.name}{{{labels_str}}} {latest.value} {int(latest.timestamp.timestamp())}")
...
class AlertManager:
    ...
    async def _trigger_alert(self, rule_name: str, rule: Dict[str, Any], metric: Metric) -> None:
        """Trigger an alert."""
        ...
        logger.warning(f"Alert triggered: {rule_name} - {metric.value} {rule['operator']} {rule['threshold']}")
    ...
    async def export_logs(self, format: str = "json") -> str:
        ...
        async with self._lock:
            if format == "json":
                ...
                lines = []
                for log in self.logs:
                    lines.append(f"{log.timestamp.isoformat()} [{log.level.value.upper()}] [{log.source}] {log.message}")

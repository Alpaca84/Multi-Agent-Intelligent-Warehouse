...
class MCPAdapter(ABC):
    ...
    async def register_tools(self, mcp_server: MCPServer) -> bool:
        ...
        try:
            ...
            logger.info(f"Registered {len(self.tools)} tools from adapter: {self.config.name}")
            return True
            
        except Exception as e:
            logger.error(f"Failed to register tools for adapter '{self.config.name}': {e}")
            return False
    ...
    async def register_resources(self, mcp_server: MCPServer) -> bool:
        ...
        try:
            ...
            logger.info(f"Registered {len(self.resources)} resources from adapter: {self.config.name}")
            return True
            
        except Exception as e:
            logger.error(f"Failed to register resources for adapter '{self.config.name}': {e}")
            return False
    ...
    async def register_prompts(self, mcp_server: MCPServer) -> bool:
        ...
        try:
            ...
            logger.info(f"Registered {len(self.prompts)} prompts from adapter: {self.config.name}")
            return True
            
        except Exception as e:
            logger.error(f"Failed to register prompts for adapter '{self.config.name}': {e}")
            return False
    ...
    def add_prompt(self, name: str, template: str, description: str = "", arguments: List[str] = None) -> bool:
        """

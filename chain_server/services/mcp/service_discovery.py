...
class ServiceRegistry:
    ...
    async def get_service_statistics(self) -> Dict[str, Any]:
        ...
        async with self._lock:
            ...
            healthy_services = sum(1 for h in self.health_status.values() if h.is_healthy)
    ...
    def _update_service_endpoints(self, service_info: ServiceInfo) -> None:
        ...
        if service_info.service_id not in self.service_endpoints[service_info.endpoint]:
            self.service_endpoints[service_info.endpoint].append(service_info.service_id)
    ...
    async def _check_service_health(self, service_id: str, service_info: ServiceInfo) -> None:
        ...
        try:
            ...
            async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=self.config.service_timeout)) as session:
...
class ServiceDiscovery:
    ...
    async def register_adapter(self, adapter: MCPAdapter, service_name: str = None) -> str:
        """Register an MCP adapter as a service."""
        service_name = service_name or adapter.config.name
        
        capabilities = []
        if hasattr(adapter, 'tools'):
            capabilities.extend([f"tool:{tool_name}" for tool_name in adapter.tools.keys()])
    ...
    async def discover_adapters(self, adapter_type: AdapterType = None) -> List[ServiceInfo]:
    ...
    async def discover_tools(self, tool_category: ToolCategory = None) -> List[DiscoveredTool]:
        ...
        for adapter_info in adapters:
            try:
                ...
                logger.error(f"Error discovering tools from adapter {adapter_info.service_id}: {e}")
    ...
    async def _notify_discovery_callbacks(self, event_type: str, service_info: ServiceInfo) -> None:

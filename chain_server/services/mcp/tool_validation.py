...
class ToolValidationService:
    ...
    async def _validate_required_parameters(
        ...
        return {
            ...
            "suggestions": [f"Provide required parameter: {param}" for param in missing_params]
        }
        ...
        for param_name, param_value in arguments.items():
            if param_name in tool.parameters:
                ...
                if "minimum" in param_schema and param_value < param_schema["minimum"]:
                    value_errors.append(f"{param_name}: value {param_value} below minimum {param_schema['minimum']}")
                
                if "maximum" in param_schema and param_value > param_schema["maximum"]:
                    value_errors.append(f"{param_name}: value {param_value} above maximum {param_schema['maximum']}")
                
                # Check enum values
                if "enum" in param_schema and param_value not in param_schema["enum"]:
                    value_errors.append(f"{param_name}: value {param_value} not in allowed values {param_schema['enum']}")
        
        return {
            "valid": len(value_errors) == 0,
            "suggestions": value_errors
        }
...
class ErrorHandlingService:
    ...
    async def get_error_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "recent_errors": len([e for e in self.error_history if (datetime.utcnow() - e.occurred_at).hours < 24])
        }

def validate_tool_execution(validation_level: ValidationLevel = ValidationLevel.STANDARD):

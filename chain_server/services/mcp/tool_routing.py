...
class ToolRoutingService:
    ...
    def __init__(self, tool_discovery: ToolDiscoveryService, tool_binding: ToolBindingService):
    ...
    async def route_tools(
        ...
        try:
            ...
            context.complexity = await self.complexity_analyzer.analyze_complexity(context.query)
            ...
            logger.info(f"Routed {len(selected_tools)} tools using {strategy.value} strategy")
            return decision
    ...
    async def _discover_candidate_tools(self, context: RoutingContext) -> List[DiscoveredTool]:
        ...
        try:
            ...
            for entity_type, entity_value in context.entities.items():
                entity_tools = await self.tool_discovery.search_tools(f"{entity_type} {entity_value}")
    ...
    def _calculate_overall_score(
        ...
        if strategy == RoutingStrategy.PERFORMANCE_OPTIMIZED:
            return (performance_score * 0.4) + (accuracy_score * 0.3) + (latency_score * 0.3)
        elif strategy == RoutingStrategy.ACCURACY_OPTIMIZED:
            return (accuracy_score * 0.5) + (capability_score * 0.3) + (context_score * 0.2)
        elif strategy == RoutingStrategy.BALANCED:
            return (performance_score * 0.25) + (accuracy_score * 0.25) + (capability_score * 0.25) + (context_score * 0.25)
        elif strategy == RoutingStrategy.COST_OPTIMIZED:
            return (cost_score * 0.4) + (performance_score * 0.3) + (accuracy_score * 0.3)
        elif strategy == RoutingStrategy.LATENCY_OPTIMIZED:
            return (latency_score * 0.5) + (performance_score * 0.3) + (accuracy_score * 0.2)
        else:
            return (performance_score + accuracy_score + capability_score + context_score) / 4.0
    
    def _calculate_tool_confidence(self, tool: DiscoveredTool, context: RoutingContext) -> float:
    ...
    def _calculate_confidence(self, tool_scores: List[ToolScore]) -> float:
        ...
        avg_confidence = sum(score.confidence for score in tool_scores) / len(tool_scores)
    ...
    def _record_routing_decision(self, decision: RoutingDecision, context: RoutingContext) -> None:
    ...
    async def get_routing_statistics(self) -> Dict[str, Any]:
        ...
        avg_confidence = sum(d["decision"].confidence for d in self.routing_history) / total_decisions
        avg_tools_selected = sum(len(d["decision"].selected_tools) for d in self.routing_history) / total_decisions
...
class CapabilityMatcher:
    ...
    async def match_capabilities(self, tool: DiscoveredTool, context: RoutingContext) -> float:
...
class ContextAnalyzer:
    ...
    async def analyze_context_relevance(self, tool: DiscoveredTool, context: RoutingContext) -> float:

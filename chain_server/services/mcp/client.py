...
class MCPClient:
    ...
    def __init__(self, client_name: str = "warehouse-assistant-client", version: str = "1.0.0"):
    ...
    async def connect_server(self, server_name: str, connection_type: MCPConnectionType, 
                           endpoint: str, **kwargs) -> bool:
        """
    ...
    async def _discover_resources(self, server_name: str) -> None:
        ...
        try:
            ...
            logger.error(f"Failed to discover resources from server '{server_name}': {e}")
    ...
    async def _discover_prompts(self, server_name: str) -> None:
        ...
        try:
            ...
            if response and "result" in response:
                ...
                logger.info(f"Discovered {len(prompts)} prompts from server '{server_name}'")
                
        except Exception as e:
            logger.error(f"Failed to discover prompts from server '{server_name}': {e}")
    
    async def _send_request(self, server_info: Dict[str, Any], request: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if server_info["connection_type"] == MCPConnectionType.HTTP:
                ...
                raise ValueError(f"Unsupported connection type: {server_info['connection_type']}")
                
        except Exception as e:
            logger.error(f"Failed to send request: {e}")
            return None
    
    async def _send_http_request(self, server_info: Dict[str, Any], request: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    async def _send_websocket_request(self, server_info: Dict[str, Any], request: Dict[str, Any]) -> Optional[Dict[str, Any]]:
